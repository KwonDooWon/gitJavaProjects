...
public class POSSystem {
	...
	public static POSSystem getPosInstance() {
		//POSSystem내부에 있는 매서드 영역이므로 private한 psoIstance에
		//접근이 가능하다.
	...
	public int printMenu() {
		...
		System.out.println("---------------");
		System.out.print("원하시는 번호를 입력해 주세요.(이외번호를 입");
	...
	public void joinCustomer() {
		for (int i = 0; i < this.CUSTOMER_LIST.length; i++) {
			if(this.CUSTOMER_LIST[this.CUSTOMER_LIST.length - 1] != null) {
				System.out.print("더이상 회원을 등록할수 없습니다.");
				return;
			}
	...
	public void login() {
		...
		for (int i = 0; i < CUSTOMER_LIST.length; i++) {
			if(CUSTOMER_LIST[i] == null)
				break;
			else if(CUSTOMER_LIST[i].equals(id, password)) {
				loginCustomer = CUSTOMER_LIST[i];
				System.out.print("로그인 성공");
				break;
			} else if (!CUSTOMER_LIST[i].equals(id)) {
